...
from ml_hitwh.model.game import Game, PlayerAndWind
...
async def _next_game_id(group_id: int, now: datetime):
...
async def new_game(create_user_id: int, group_id: int, player_and_wind: PlayerAndWind) -> Game:
    now = datetime.now(tzlocal.get_localzone())
    game_id = await _next_game_id(group_id, now)
